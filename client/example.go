...
import (
	"fmt"
	kg "pqgch-client/gake"
)

func Example() [64]byte {
	numParties := 3

	kem_a := kg.GetKemKeyPair()
	kem_b := kg.GetKemKeyPair()
	kem_c := kg.GetKemKeyPair()

	var parties = make([]kg.Party, numParties)
	...
	for i := 0; i < numParties; i++ {
		...
		parties[i].Commitments = make([]kg.Commitment, numParties)
	...
	for i := 0; i < numParties; i++ {
		...
		ake_senda, tk, eska := kg.KexAkeInitA(parties[right].Pk[:])
		ake_sendb, kb := kg.KexAkeSharedB(ake_senda, parties[right].Sk[:], parties[i].Pk[:])
		ka := kg.KexAkeSharedA(ake_sendb, tk, eska, parties[i].Sk[:])

		copy(parties[i].KeyRight[:], ka[:])
		copy(parties[right].KeyLeft[:], kb[:])

		ake_senda2, tk2, eska2 := kg.KexAkeInitA(parties[left].Pk[:])
		ake_sendb2, kb2 := kg.KexAkeSharedB(ake_senda2, parties[left].Sk[:], parties[i].Pk[:])
		ka2 := kg.KexAkeSharedA(ake_sendb2, tk2, eska2, parties[i].Sk[:])
	...
	for i := 0; i < numParties; i++ {
		xi, ri, commitment := kg.ComputeXsCommitment(
			i,
			parties[i].KeyRight,
			parties[i].KeyLeft,
			parties[i].Pk)
	...
	for i := 0; i < numParties; i++ {
		masterkey := kg.ComputerMasterKey(numParties, i, parties[i].KeyLeft, parties[i].Xs)
		copy(parties[i].MasterKey, masterkey)
	}

	for i := 0; i < numParties; i++ {
		//kg.PrintParty(parties[i])
	}
	sksid := make([][64]byte, numParties)

	// Compute sk_sid
	for i := 0; i < numParties; i++ {
		sksid[i] = kg.ComputeSkSid(numParties, parties[i].MasterKey, pids)
	}
